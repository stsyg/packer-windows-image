...
resource "azurerm_resource_group" "packer_sig" {
  location = var.location
  name     = "${var.prefix}-${var.service}-infra-rg"
...
resource "azurerm_shared_image_gallery" "this" {
  ...
  resource_group_name = azurerm_resource_group.packer_sig.name
  location            = azurerm_resource_group.packer_sig.location
...
resource "azurerm_role_definition" "aibIdentity" {
  ...
  assignable_scopes = [
    ...
    azurerm_resource_group.packer_sig.id,
    data.azurerm_subscription.this.id, # /subscriptions/00000000-0000-0000-0000-000000000000
  ]
}
